...
public class VerticalScrollTextView extends RelativeLayout {
    ...
    private List<String> dataOriginSource;
    ...
    private void init(Context context) {
        ...
        mTvContentTop.setGravity(RelativeLayout.CENTER_VERTICAL);
        mTvContentBottom.setGravity(RelativeLayout.CENTER_VERTICAL);

        addView(mTvContentTop);
        addView(mTvContentBottom);
    ...
    private void resetData() {
        ...
        if (dataOriginSource != null && dataOriginSource.size() != 0){

            String total = mDataSource.get(0) + dataOriginSource.get(0);

            SpannableStringBuilder builder = new SpannableStringBuilder(total);
            builder.setSpan(new StrikethroughSpan(),mDataSource.get(0).length(),total.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            builder.setSpan(new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.colorAAAAAA)), mDataSource.get(0).length(), total.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            builder.setSpan(new RelativeSizeSpan(0.5f),mDataSource.get(0).length(),total.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            mTvContentTop.setText(builder);
        }else {
    ...
    private void autoSlider() {
        ...
        if (dataOriginSource != null && dataOriginSource.size() != 0){
            ...
            String total = mDataSource.get(mCurrentItemIndex) + dataOriginSource.get(mCurrentItemIndex);

            SpannableStringBuilder builder = new SpannableStringBuilder(total);
            builder.setSpan(new StrikethroughSpan(),mDataSource.get(mCurrentItemIndex).length(),total.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            builder.setSpan(new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.colorAAAAAA)), mDataSource.get(mCurrentItemIndex).length(), total.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            builder.setSpan(new RelativeSizeSpan(0.5f),mDataSource.get(mCurrentItemIndex).length(),total.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        if (dataOriginSource != null && dataOriginSource.size() != 0){
            ...
            mTvContentBottom.setText(builder);
    ...
    public void setDataSource(List<String> dataSource, List<String> dataOriginSource) {
        this.mDataSource = dataSource;
